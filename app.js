/* eunoia full app.js — complete heavy editor SPA
   - SPA routing
   - projects (create/open/delete)
   - centered Google-Docs style editor
   - toolbar with formatting
   - TOC (H1/H2/H3)
   - sticky notes (per project)
   - reverie (characters + plots per project)
   - Spotify embed (melomanie)
   - export docx/pdf
   - autosave per-project using localStorage
*/
...
// ---------- storage keys ----------
const PROJECTS_KEY = 'eunoia_projects_v4';
const PROJ_PREFIX = 'eunoia_proj_v4_'; // + id
...
function saveNow(silent=false){
  ...
  saveTimer = setTimeout(()=>{
    ...
    const data = loadProjectContent(currentProjectId) || {title: $('#projectTitleInput').value || 'Untitled', content:'', notes:[], characters:[], plots:[]};
    data.title = $('#projectTitleInput').value || data.title;
...
// Hook up reverie tab buttons to show/hide character/plot sections
...
if(revCharBtn && revPlotBtn && revCharsSection && revPlotsSection){
  function showCharacters() {
    revCharsSection.classList.remove('hidden');
    revPlotsSection.classList.add('hidden');
  }
  function showPlots() {
    revPlotsSection.classList.remove('hidden');
    revCharsSection.classList.add('hidden');
  }
  revCharBtn.addEventListener('click', showCharacters);
  revPlotBtn.addEventListener('click', showPlots);
}

// characters & plots helper functions (project-scoped)
